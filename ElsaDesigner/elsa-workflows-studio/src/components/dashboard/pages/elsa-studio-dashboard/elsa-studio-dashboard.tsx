import { Component, h, Prop, getAssetPath, Method } from '@stencil/core';
...
export class ElsaStudioDashboard {
  ...
  async menuItemSelected(item: DropdownButtonItem) {
    if (item.value == 'logout') {
      await state.keycloak.logout({redirectUri: window.location.href});
    }
  }
  ...
  render() {
    ...
    let menuItems = (this.dashboardMenu.data != null ? this.dashboardMenu.data.menuItems : []).map(([route, label]) =>
      this.i18next.exists(`${menuItemsNamespace}:${route}`) ? [route, this.i18next.t(`${menuItemsNamespace}:${route}`)] : [route, label],
    );
      ...
      if (state.keycloak.authenticated) {
        return (
          <elsa-dropdown-button
            text={`Hello ${state.userProfile.firstName} ${state.userProfile.lastName}`}
            items={ddlitems}
            btnClass="elsa-bg-gray-800 elsa-text-gray-300 elsa-w-full   elsa-shadow-sm elsa-px-4 elsa-py-2 elsa-inline-flex elsa-justify-center elsa-text-sm elsa-font-medium"
    ...
    return (
      <div class="elsa-h-screen elsa-bg-gray-100">
            ...
            <div class="elsa-flex elsa-items-center elsa-justify-between elsa-h-16">
              <div class="elsa-flex elsa-items-center">
                ...
                <div class="hidden md:elsa-block">
                  <div class="elsa-ml-10 elsa-flex elsa-items-baseline elsa-space-x-4">{menuItems.map(item => renderFeatureMenuItem(item, basePath))}</div>
                </div>
              </div>
        ...
        <main>
          <stencil-router>
            <stencil-route-switch scrollTopOffset={0}>{routes.map(item => renderFeatureRoute(item, basePath))}</stencil-route-switch>
          </stencil-router>
        </main>
      </div>
    );
